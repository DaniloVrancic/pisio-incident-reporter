...
public class AnalysisService {
    ...
    public List<Cluster> createClusters(List<Location> locations, String date) {
        ...
        for(Cluster cluster : clusters.values()){
            double lat = cluster.items.stream().map(loc -> loc.incident.getLatitude().doubleValue()).reduce(Double::sum).get();
            double lng = cluster.items.stream().map(loc -> loc.incident.getLongitude().doubleValue()).reduce(Double::sum).get();
